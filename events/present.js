...
module.exports = async (client, message) => {
  ...
  if (presentationChannel && !message.member.roles.cache.has(presentedRole)) {
    ...
    const embed = {
      ...
      description:
        'Caso você tenha errado a digitação de algo em sua mensagem de apresentação, basta digitar o comando `o!resetar` no *chat de comandos do servidor* para resetar a sua apresentação!',
    }
    message.author
      .send({ embed: embed })
      .catch(() =>
